...
Get file status information, such as size (in bytes), permissions, and timestamps.
Returns an object with file metadata.

<Aside type='caution'> 
Currently only `size` field is supported.
</Aside>

> Check [fstat(3p)](http://man7.org/linux/man-pages/man3/fstat.3p.html) for more details.
...
// You can use it to read whole file
